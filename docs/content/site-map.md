...
`src/app/proxy.ts` `PUBLIC_ROUTES` includes all public marketing, legal, auth
and static artifact routes: `["/", "/services", "/work", "/how-we-work", "/about",
"/start-a-project", "/login", "/invite/*", "/auth/*", "/favicon.ico",
"/design-system", "/legal/*", "/privacy", "/terms", "/cookies", "/robots.txt",
"/sitemap.xml", "/llms.txt", "/index.md", "/.well-known/security.txt",
"/.well-known/humans.txt", "/manifest.webmanifest"]`. Any new public route must
be added there and to `STATIC_PUBLIC_ROUTES` if it should bypass locale routing.

### SEO / AI-readiness artifacts

| Artifact          | Route                        | Purpose                                                                                |
| ----------------- | ---------------------------- | -------------------------------------------------------------------------------------- |
| robots.txt        | `/robots.txt`                | Content Signals (`ai-train=allow`, `search=yes`, `ai-input=allow`) + AI crawler policy |
| sitemap.xml       | `/sitemap.xml`               | All 17 locales × 10 public paths with hreflang + `x-default`                           |
| llms.txt          | `/llms.txt`                  | AI crawler index of public pages with Markdown-version links                           |
| Markdown fallback | `/[locale]/index.md`         | Content negotiation: serves clean Markdown on `Accept: text/markdown`                  |
| security.txt      | `/.well-known/security.txt`  | Security contact + policy                                                              |
| humans.txt        | `/.well-known/humans.txt`    | Studio credits (no tech-stack disclosure)                                              |
| Web App Manifest  | `/manifest.webmanifest`      | PWA metadata (name, icons, theme colors)                                               |
| OpenGraph images  | `/brand/sdk-thumbnail-*-png` | 1200×628 social thumbnails                                                             |
...
| `Header` / `PublicHeader`                                                                                          | `src/components/layout/{Header,PublicHeader}.tsx` — `Header` is the interactive client component (Auth0 `useUser`); `PublicHeader` wraps it as a Server Component for marketing routes using `getCurrentPrincipal()` to avoid pulling the Auth0 client bundle into public pages. Approved logo, 11px uppercase nav, mobile menu. |
