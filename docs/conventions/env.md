...
| `MAIL_SMTP_URL`         | no         | Development only: SMTP URL of the local mail sink (`packages/tooling/src/mail/mail-sink.ts`, smtp-tester) that enquiry and invitation emails are delivered to. Defaults to `smtp://localhost:1025`. The sink auto-starts with `pnpm run dev`; run standalone with `pnpm run mail`. Received email is checked via the local inbox UI (`pnpm run mail:ui`), the `pnpm run mail:*` CLI commands, or the `maildev` MCP server. Ignored in production. |
| `MAIL_HTTP_URL`         | no         | Development only: HTTP base URL of the mail sink API. Defaults to `http://localhost:1080`. Read by the mail opener (`packages/tooling/src/mail/mail-open.ts`), the mail CLI (`packages/tooling/src/mail/mail-cli.ts`), and the `maildev` MCP server (`packages/tooling/src/mail/mail-mcp.ts`). The inbox UI is served from this origin (loopback only).                                                                                           |
