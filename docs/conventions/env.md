...
| `MAIL_SMTP_URL`         | no         | Development only: SMTP URL of the shared local mail sink (`@sdk-e/mailbox`, smtp-tester) that enquiry and invitation emails are delivered to. Defaults to `smtp://localhost:11025`. The sink is a standalone shared service — start it once with `pnpm dlx @sdk-e/mailbox` (it is no longer spawned by `pnpm run dev`). Received email is checked via the inbox UI (`pnpm dlx @sdk-e/mailbox open`), the `pnpm dlx @sdk-e/mailbox` CLI commands, or the `maildev` MCP server. Ignored in production. |
| `MAIL_HTTP_URL`         | no         | Development only: HTTP base URL of the mailbox HTTP API. Defaults to `http://localhost:11090`. Read by the CLI (`mailbox-cli`) and the `maildev` MCP server. The inbox UI is served from this origin (loopback only).                                                                                                                                                                                                                                                                                |
| `MAIL_SMTP_PORT`        | no         | Development only: SMTP port the mailbox SMTP server binds to. Defaults to `11025`. If changed, `MAIL_SMTP_URL` must point at the new port.                                                                                                                                                                                                                                                                                                                                                           |
| `MAIL_HTTP_PORT`        | no         | Development only: HTTP port the mailbox API listens on. Defaults to `11090`. If changed, `MAIL_HTTP_URL` must point at the new port.                                                                                                                                                                                                                                                                                                                                                                 |
...
```bash
pnpm --filter @platform/db exec prisma dev --detach -n app -P 5432
```
...
```bash
pnpm --filter @platform/db exec prisma dev ls
```

Manage the local server lifecycle:

```bash
pnpm --filter @platform/db exec prisma dev ls        # list servers (name, status, URLs)
pnpm --filter @platform/db exec prisma dev stop      # stop the current project's server
pnpm --filter @platform/db exec prisma dev start     # restart a stopped server
```

Then create the `app` database on that server (once):

```bash
DATABASE_URL="<tcp URL from prisma dev ls>" pnpm --filter @platform/db exec prisma db execute --stdin <<'SQL'
CREATE DATABASE app;
SQL
```
...
`<port>` with the value from `pnpm --filter @platform/db exec prisma dev ls`:
...
```bash
DATABASE_URL="$(node -e "require('dotenv').config({path:'.env.local'});process.stdout.write(process.env.DATABASE_URL)")" pnpm --filter @platform/db exec prisma migrate deploy
```
