...
export default async function CompanyRequestsPage({
  ...
  const [requests, t, tr] = await Promise.all([
    ...
    <section>
      ...
      <div className="mt-10 space-y-4">
        {requests.map((request) => (
          ...
          >
            <Card className="flex items-center justify-between gap-4 transition-colors hover:border-dark">
              <div>
                <h2 className="text-h3 font-extrabold">{request.title}</h2>
                <p className="mt-2 text-body text-muted-foreground">
                  {tr(`capabilities.${request.capability}`)}
                </p>
              </div>
              <Badge>
