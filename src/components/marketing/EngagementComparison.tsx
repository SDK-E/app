  ...
  return (
    <div className="overflow-hidden rounded-card border border-line bg-paper">
      ...
      {["bestFor", "output", "commitment"].map((field) => (
        <div key={field} className="grid border-b border-line last:border-b-0 lg:grid-cols-[0.55fr_repeat(3,1fr)]">
          <p className="bg-light p-5 text-label font-bold uppercase tracking-eyebrow">
            {labels[field as keyof typeof labels]}
          </p>
          {options.map((option) => (
            <div key={option.title} className="border-t border-line p-5 first:border-t-0 lg:border-l lg:border-t-0">
