...
export function ServiceChapter({
  ...
  return (
    <article
      id={item.id}
      className="scroll-mt-8 border-t-2 border-current py-10 first:border-t-0 first:pt-0 lg:py-14"
    >
      <div className="grid gap-8 lg:grid-cols-[0.65fr_1.35fr] lg:gap-[70px]">
        <div>
          <p className="text-label font-bold uppercase tracking-eyebrow">{item.number}</p>
          <h3 className="mt-4 max-w-[16ch] text-[30px] font-extrabold tracking-title md:text-[38px]">
            {item.title}
          </h3>
          <p className="mt-5 max-w-[48ch] text-body">{item.situation}</p>
        </div>
        <div className="grid gap-8 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2 grid gap-5 border-t border-line pt-6 sm:grid-cols-[1fr_1.2fr]">
            <div>
              ...
              <p className="mt-3 text-body">{item.evidence.join(" · ")}</p>
            </div>
