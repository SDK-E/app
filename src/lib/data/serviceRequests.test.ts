...
describe("request workflow", () => {
  it("allows SDK review only from submitted", () => {
    expect(resolveSdkTransition("SUBMITTED", { decision: "start-review" })?.toStatus).toBe("IN_REVIEW");
    expect(resolveSdkTransition("DRAFT", { decision: "start-review" })).toBeNull();
  });

  it("requires the correct source state for a proposal", () => {
    const decision = { decision: "proposal-ready" as const, content: "A bounded assessment with written findings and a prioritized implementation path." };
