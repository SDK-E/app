...
function assertClientRoleGrant(principal: AppPrincipal, role: ClientRole) {
  ...
  if (
    role === "ADMINISTRATOR" &&
    (principal.kind !== "client" || principal.role !== "OWNER") &&
...
export async function updateMembershipRole(
  ...
  if (principal.kind === "client" && membership.companyId !== principal.companyId)
    forbidden("Cross-company access is denied.");
  if (principal.kind === "client" && membership.userId === principal.id)
    forbidden("You cannot change your own role.");
...
export async function requestCompanyAccess(
  ...
  if (principal.kind !== "unassigned")
    forbidden("Only unassigned users can request access to a company.");
...
export async function listCompanyAccessRequests(
  ...
  return getPrisma().companyAccessRequest.findMany({
    where: { companyId: input?.companyId, status },
    include,
    orderBy: { createdAt: "desc" },
  });
}
